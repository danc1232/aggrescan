...
def main():
    ...
    try:
        ...
        if DEBUG:
            print(f'[{c.YELLOW}X{c.RES}]\tCancelling scan -- KeyboardInt / Cancelled')
        else:
            print(f'[{c.YELLOW}X{c.RES}]\tCancelling scan.')
        shutdown()
    except RuntimeError:
        if DEBUG:
            print(f'[{c.YELLOW}X{c.RES}]\tCancelling scan -- RuntimeError')
        else:
            print(f'[{c.YELLOW}X{c.RES}]\tCancelling scan.')
        shutdown()
    except SystemExit:
        ...
        print(f'[{c.YELLOW}X{c.RES}]\tError -- Shutting down Aggrescan.')
        if DEBUG: traceback.print_exc()
        shutdown()

# windows specific thing here to prevent annoying errors on keyboard interrupts
def OS_requirements():
        if system == 'Windows':
